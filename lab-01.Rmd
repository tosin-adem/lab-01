...
```{r Ex_d}
visitors %>% filter(n_2022 == (n=565772))
```
...
visitors %>% 
  summarise(
    mean_2021 = mean(is.na(n_2021), trim = 0, na.rm = FALSE),
    med_2021 = median(is.na(n_2021), na.rm = FALSE)
  )
```
...
Not available; there is missing values
...
Data is skewed towards the right-- most data falls towards right therefore graph of values is asymmetrical. 
